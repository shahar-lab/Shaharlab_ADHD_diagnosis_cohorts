...
#load 
patas <- read_tsv("data/תשפה/collected_data/PATAS_תשפה_June+1,+2025_11.53_values.tsv",
...
patas <- patas |>
  ...
  filter(Finished == 1, !is.na(patas1)) |> 
...
# 1. Examine subjectid duplicates
    ...
    patas |> group_by(subjectid) |> filter(n() > 1) |> ungroup() |> View()

# 2. Retain only the first line for those how complete the whole thing more then once
    patas = patas |> group_by(subjectid) |> slice_min(order_by = date_recorded, n = 1) |> ungroup()
...
#### SAVE ----
save(patas, file = 'data/תשפה/raw_data/patas.Rdata')
