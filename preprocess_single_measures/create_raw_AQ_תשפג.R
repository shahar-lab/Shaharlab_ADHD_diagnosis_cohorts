...
#load 
aq <- read_tsv("data/תשפג/collected_data/AQ+template+-+עברית+-+Copy_October+1,+2025_10.01_values.tsv",
...
# 1. Examine subjectid duplicates
    ...
    #aq |> group_by(subjectid) |> filter(n() > 1) |> ungroup() |> View()
    
    # Retain only the first line for those how complete the whole thing more then once
    aq = aq |> group_by(subjectid) |> slice_min(order_by = date_recorded, n = 1) |> ungroup()
...
#### SAVE ----
save(aq, file = 'data/תשפג/raw_data/aq.Rdata')
